                ...
                <table class="w-full table-auto">
                    <thead>
                        <tr class="bg-gray-100 text-gray-700 uppercase text-sm leading-normal">
                            ...
                            <th class="py-3 px-6 text-center font-semibold">Orden</th>
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                        <?php foreach ($sliders as $index => $slider): ?>
                            <tr class="border-b border-gray-200 odd:bg-white even:bg-gray-50 hover:bg-gray-100 transition-colors duration-150">
                                ...
                                <td class="py-3 px-6 text-center">
                                    <?php if ($slider['img']): ?>
                                        <img src="/<?= esc($slider['img']) ?>" alt="Slider Image" class="h-12 w-12 object-cover rounded-md shadow-sm mx-auto">
                                    <?php else: ?>
                                        <span class="text-gray-400">No imagen</span>
                                    <?php endif; ?>
                                </td>
                                <td class="py-3 px-6 text-center" data-orden="<?= esc($slider['orden']) ?>"><?= esc($index + 1) ?></td>
                                <td class="py-3 px-6 text-center">
                                    ...
                                    <?php else: ?>
                                        <span class="bg-red-100 text-red-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">Inactivo</span>
                                    <?php endif; ?>
                                </td>
                                <td class="py-3 px-6 text-center">
                                    <div class="flex item-center justify-center space-x-3">
                                        <a href="/slider/edit/<?= esc($slider['id']) ?>" class="text-blue-600 hover:text-blue-800 font-medium transition-colors duration-150">
                                            Editar
                                        </a>
                                        <a href="/slider/delete/<?= esc($slider['id']) ?>" 
                                           onclick="return confirm('¿Estás seguro de que quieres eliminar este slider?');"
