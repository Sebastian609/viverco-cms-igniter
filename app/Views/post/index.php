...
<div class="px-4">
    <h1 class="font-bold text-xl text-gray-800 text-left mb-8">Lista de Posts</h1>
    ...
    <div class="border-2 rounded-md">
        <div class="flex justify-between p-8">
            ...
            <form class="flex justify-between">
                <input type="text" class="bg-gray-100 px-6 py-3 rounded-l-md border-2 " placeholder="Buscar Por texto principal" />
                <button type="submit" class="bg-blue-600 rounded-r-md text-white h-full aspect-square">
                    <i class="fa-solid fa-magnifying-glass text-white"></i>
                </button>
            </form>

            <a href="/post/create" class="bg-blue-600 text-white flex justify-center items-center rounded-md px-3 py-2">
                Nuevo Post
            </a>
            ...
            <table class="w-full table-auto">
                <thead>
                    <tr class="text-gray-600 border-y-2 uppercase text-sm leading-normal">
                        ...
                        <th class="py-3 px-6 text-left font-semibold">Copy</th>
                ...
                <tbody id="post-table-body" class="text-gray-600 text-sm font-light">
                    <?php foreach ($posts as $index => $post): ?>
                        <tr class="draggable border-b border-gray-200 odd:bg-white even:bg-gray-50 hover:bg-gray-100 transition-colors duration-150"
                            data-id="<?= esc($post['id']) ?>">
                            <td class="py-3 px-6 text-left whitespace-nowrap"><i class="fa-solid fa-grip-vertical cursor-grab"></i></td>
                            <td class="py-3 px-6 text-left">
                                <?= esc(!empty($post['title']) ? $post['title'] : 'Sin título') ?>
                            </td>
                            <td class="py-3 px-6 text-left">
                                <?= esc(!empty($post['copy']) ? $post['copy'] : 'Sin copy') ?>
                            </td>
